...
final Parser<CombineParser> combineLexer =
    (string('combine(') | string('.combine(')).map(
      (_) => CombineParser.empty(),
    );

/// identifies toQuantitys
final Parser<ToQuantityParser> toQuantityLexer =
    (string('toQuantity(') | string('.toQuantity(')).map(
      (_) => ToQuantityParser.empty(),
    );

/// identifies convertsToQuantitys
final Parser<ConvertsToQuantityParser> convertsToQuantityLexer =
    (string('convertsToQuantity(') | string('.convertsToQuantity(')).map(
      (_) => ConvertsToQuantityParser.empty(),
    );
...
/// identifies alls
final Parser<AllParser> allLexer = (string('all(') | string('.all(')).map(
  (_) => AllParser.empty(),
);

/// identifies subsetOfs
final Parser<SubsetOfParser> subsetOfLexer =
    (string('subsetOf(') | string('.subsetOf(')).map(
      (_) => SubsetOfParser.empty(),
    );

/// identifies supersetOfs
final Parser<SupersetOfParser> supersetOfLexer =
    (string('supersetOf(') | string('.supersetOf(')).map(
      (_) => SupersetOfParser.empty(),
    );
...
final Parser<ExtensionParser> extensionLexer =
    (string('extension(') | string('.extension(')).map(
      (_) => ExtensionParser.empty(),
    );
...
final Parser<IndexOfParser> indexOfLexer =
    (string('indexOf(') | string('.indexOf(')).map(
      (_) => IndexOfParser.empty(),
    );

/// identifies substrings
final Parser<SubstringParser> substringLexer =
    (string('substring(') | string('.substring(')).map(
      (_) => SubstringParser.empty(),
    );

/// identifies startsWiths
final Parser<StartsWithParser> startsWithLexer =
    (string('startsWith(') | string('.startsWith(')).map(
      (_) => StartsWithParser.empty(),
    );

/// identifies endsWiths
final Parser<EndsWithParser> endsWithLexer =
    (string('endsWith(') | string('.endsWith(')).map(
      (_) => EndsWithParser.empty(),
    );

/// identifies containsFunctions
final Parser<ContainsFunctionParser> containsFunctionLexer =
    (string('contains(') | string('.contains(')).map(
      (_) => ContainsFunctionParser.empty(),
    );

/// identifies replaces
final Parser<ReplaceParser> replaceLexer =
    (string('replace(') | string('.replace(')).map(
      (_) => ReplaceParser.empty(),
    );
...
/// identifies takes
final Parser<TakeParser> takeLexer = (string('take(') | string('.take(')).map(
  (_) => TakeParser.empty(),
);

/// identifies intersects
final Parser<IntersectParser> intersectLexer =
    (string('intersect(') | string('.intersect(')).map(
      (_) => IntersectParser.empty(),
    );

/// identifies excludes
final Parser<ExcludeParser> excludeLexer =
    (string('exclude(') | string('.exclude(')).map(
      (_) => ExcludeParser.empty(),
    );
