...
/// identifies divSigns
final Parser<DivSignParser> divSignLexer = char(
  '/',
).map((_) => DivSignParser());

/// identifies divStrings
final Parser<DivStringParser> divStringLexer = string(
  ' div ',
).map((_) => DivStringParser());
...
final Parser<StringConcatenationParser> stringConcatenationLexer = char(
  '&',
).map((_) => StringConcatenationParser());
...
/// identifies greaters
final Parser<GreaterParser> greaterLexer = char(
  '>',
).map((_) => GreaterParser());
...
final Parser<GreaterEqualParser> greaterEqualLexer = string(
  '>=',
).map((_) => GreaterEqualParser());

/// identifies lessEquals
final Parser<LessEqualParser> lessEqualLexer = string(
  '<=',
).map((_) => LessEqualParser());

/// identifies equalss
final Parser<EqualsParser> equalsLexer = (char('=').trim() | char('=')).map(
  (_) => EqualsParser(),
);

/// identifies equivalents
final Parser<EquivalentParser> equivalentLexer = string(
  '~',
).map((_) => EquivalentParser());

/// identifies notEqualss
final Parser<NotEqualsParser> notEqualsLexer = string(
  '!=',
).map((_) => NotEqualsParser());
...
final Parser<ContainsOperatorParser> containsOperatorLexer = string(
  ' contains ',
).map((_) => ContainsOperatorParser());
...
final Parser<OrStringParser> orStringLexer = string(
  ' or ',
).map((_) => OrStringParser());

/// identifies implies
final Parser<ImpliesParser> impliesLexer = string(
  ' implies ',
).map((_) => ImpliesParser());
