...
class IifParser extends FunctionParser {
  ...
  List execute(List results, Map<String, dynamic> passed) {
    ...
    final criterionCollection = criterionExpressionParser.execute(
      results.toList(),
      passed,
    );
  ...
  List execute(List results, Map<String, dynamic> passed) => results.isEmpty
      ...
      : results.first == true ||
            ...
            ['true', 't', 'yes', 'y', '1', '1.0'].indexWhere(
                  (element) =>
                      element == results.first.toString().toLowerCase(),
                ) !=
                -1
      ? [true]
      : results.first == false ||
...
class ToQuantityParser extends FunctionParser {
  ...
  List execute(List results, Map<String, dynamic> passed) => results.isEmpty
      ...
      ? [
          ValidatedQuantity(
            value: UcumDecimal.fromString(results.first.toString()),
            unit: '1',
          ),
        ]
